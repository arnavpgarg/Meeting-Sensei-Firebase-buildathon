...
async function runAnalysis(
  ...
  if (!transcript || transcript.trim().length < 20) {
    return {
      data: null,
      error:
        'Transcript is too short. Please provide a more detailed transcript.',
    };
  }
  try {
    ...
    const errors = results
      .map((r, i) =>
        r.status === 'rejected' ? `Task ${i} failed: ${r.reason}` : null
      )
      .filter(Boolean);
    ...
    const [summary, keyDecisions, category, sentiment, timeline, teamTasks] =
    ...
    const analysisData: Analysis = {
      ...
      timeline:
        timeline || {
          timeline: [],
          ganttChartMarkdown: 'Timeline could not be generated.',
        },
...
export async function analyzeVideo(videoDataUri: string, language?: string) {
  try {
